...
**How to read this document:** sections follow the layers of the app:
...
- Desktop with no room open shows the low-key placeholder (seal + "Open a letter,
  or start a new one.").
...
(secondary), plus a low-key "Back to your rooms" escape that only exists when
rooms exist. Footer carries the trust line: "Messages are encrypted in your
browser and destroyed on schedule."

- `[@media(max-height:720px)]` compaction keeps everything above the fold on short screens.
...
to reveal). **The passphrase is never React state**: it is seeded straight
...
320px sidebar on desktop. Header: title, low-emphasis `+` New room icon button,
...
| Burned | Ash cards (below), cleared from storage on next full reload |
...
- `ChatHeader` (below)
- Message scroll area: `role="log"` + `aria-live="polite"`, `.scroll-quiet` scrollbar,
  `max-w-[720px]` column. Auto-scrolls to newest on arrival **unless** the user
  is scrolled up >220px, in which case the jump pill counts what arrived below the fold
...
- **`TimeAwareMessages`**: walks grouped messages and inserts a small
...
| Reply slip (round 29) | The quoted words above the input (alias + snippet + X to cancel); beginning a reply focuses the input, touch included (the reply IS an intent to write); submit encrypts the quote snapshot into the message (`makeReplySnapshot` at send time, so the quote is exactly what was on screen); **Escape releases the quote first** (stopPropagation keeps the window-level escape-to-rooms handler from seeing the key) |
...
`t-body` above the file card. The caption is carried by the meta frame's canonical-
...
wire side: `sealReact` frames carry the mark glyph in the canonical-signed `text`
field and the target in `messageId` (see `room-protocol.ts`); one mark per sender
...
**`SystemLine`**: low-contrast centered meta text ("Quiet Heron joined.", "A message
...
single object URL is created in an effect keyed on the message id and revoked
...
is no longer the file on screen; the effect itself only creates and revokes.
...
- **PDF**: rendered by **pdfjs-dist v6 onto a `<canvas>`** (worker vendored
  ...
  fall back to an `<iframe>` with the blob URL, but
  **view-once files stay unopened**: the notice says falling back would expose
...
- **Text**: escaped plain text in a monospace `pre` (React escaping, never
  ...
  is never executed here"; SVG renders in an `<img>` context, where its scripts
  cannot run.
...
boxes stay full-width `break-all` mono, so long passphrases never wrap the
...
whitespace and never crowding it, members (not the creator, since they already
hold burn) get a low-emphasis **Report this room** row: Flag glyph + label in
...
The key-derivation moment: a seal-press animation, not a spinner. A 96px
...
slowness is real: argon2id with 64 MB of memory hardness
genuinely takes a moment, and the store enforces `MIN_SEAL_MS = 1700` so
the animation never finishes before the crypto does. `role="status"` +
`aria-live="polite"`.
...
Single-message burns are subtler: the bubble's `msg-burning` (ember rim →
...
flecks rise up-and-right, off the line of text, in the direction writing
leaves. The product metaphors: the drop is the writing, the flecks
...
- `intact` (default): the seated drop with its flecks rising. With
  `breathe`, the flecks rise and fade in low amplitude on a staggered loop
...
Removal is a two-step sequence: `dismiss` marks it leaving (the component
...
a test notification), denied (plain copy pointing at browser site
...
and the finality is stated plainly: "Reporting cannot unsend anything,
...
- Consumers: bottom sheets (`bottom-[var(--kb-inset,0px)]`,
  ...
  shell (`pb-[var(--kb-inset,0px)]`, so composer, typing strip, and message
...
- Division of labor: **iOS** (overlays the page) is fully handled by the
  ...
  and the two mechanisms never double-count.
...
(Nightfall: `#17181A` family, forest `#4A6453`: a dark variant of the same desk,
...
- iOS input-zoom guard: `@supports (-webkit-touch-callout: none) and
  ...
  to outrank Tailwind; this kills Safari's focus-zoom on 15/15.5px fields.
...
reply?)` (sign → pad → encrypt → relay, optimistic; the reply snapshot is carried by
the frame and the optimistic view as `replyTo`; files become 1 meta + 44
uniform chunk frames), `emitTyping`
(throttled 2.5s, sent over the same encrypted frames), `spendViewOnce`,
...
then renders. Forged or replayed traffic becomes either a plain rejection
...
armed by `member:presence(false)` and by the `room:state` diff
(registry members absent from the relay's live list are marked offline,
which also fixes the stale away-dot after a refresh). When a clock expires,
