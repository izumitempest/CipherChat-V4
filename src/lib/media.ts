// Images that carry their own biography.
//
...
// CipherChat seals whatever bytes it is given. For a product whose
// promise is "a conversation that leaves no trace", shipping a
// photo that carries its own return address inside the ciphertext
...
// re-encoded through a canvas (pixels only; the canvas knows nothing
// of the file's past) and it is the RE-ENCODED bytes that get sealed.
// If the re-encode fails on a file that carried metadata, the file is
// not attached at all — the honest failure is a notice, never a
// silent send of location data.
//
// Formats deliberately out of scope, and why: SVG carries no EXIF and
...
// not GPS-shaped. Both pass through.
 ...
 * comments; the file's biography stays on the sender's machine.
 * Returns null when the browser cannot decode or re-encode it (the
 * caller's honest failure path). JPEG/WebP re-encode at quality
