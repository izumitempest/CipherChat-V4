...
- **Anyone who knows a room ID can end it** (Task 31, stated as a ceiling rather than hidden): the abuse-report endpoint terminates a room by ID with no proof of membership — the only act of moderation a blind-relay architecture permits, and the same griefing ceiling a leaked creator token already had. Room IDs are unguessable in practice and the endpoint is tightly rate-limited per IP, with existence never confirmed to strangers; a room killed this way is indistinguishable to its members from a creator's burn. There is no content to review, no member to suspend, no history to scrub — the operator contact (`abuse@cipherchat.app`, README + Settings → About) and the endpoint are the whole abuse surface.
