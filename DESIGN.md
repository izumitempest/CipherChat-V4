...
| **Quote flash (`quote-flash`)** | 1300ms, once | a soft ring of forest breathes twice and lets go — recognition, not alarm | static 8% forest tint |
...
- **A quote is a copy, not a reference.** The reply carries its own snapshot of the quoted words, sealed inside its frame at compose time — so the quote still reads after the original burns. Tapping it jumps to the original while it lives in session memory, and says so quietly when it doesn't ("That message is no longer in this session").
- **View-once targets are quoted as "Sealed message."** A quote is a preview, and a sealed card promises no previews — the reply never leaks what opening would have shown.
- **Files open in the app, never into the browser's hands.** Decrypted bytes live in memory behind a revocable blob URL; the viewer decides what "opening" means per type — zoom, play, render, read, inspect bytes.
- **A view-once file has no download anywhere** — not on the card, not in the viewer. Viewing is the point; the plaintext never touches the disk through us. (The standing limit holds: a determined member can still screenshot — the policy removes our hand from the till, not physics.)
...
- **Client** (`/`, hash-routed `#/`, `#/new`, `#/join/:id`, `#/rooms`, `#/r/:id`): all crypto in WebCrypto + hash-wasm + @noble/curves.
  ...
  - **Image metadata — EXIF stripping** (Task 31; `src/lib/media.ts` + the composer's attach path): a photo must not carry its own return address. Before an image is attached, its bytes are scanned by a **pure, byte-level detector** for the segments that hold a photograph's biography — JPEG `APP1` (EXIF **and** XMP packets) and `COM` free-text comments, PNG `eXIf`/`tEXt`/`iTXt`/`zTXt`/`tIME` chunks, WebP `EXIF`/`XMP` chunks. **Clean files pass through untouched** — no re-encode, no quality loss, no cost (most screenshots and downloads are already clean). Files that carry metadata are **re-encoded through a canvas** (decode → draw → encode: JPEG/WebP at quality 0.92, PNG losslessly) and it is the re-encoded bytes that get sealed — the canvas knows nothing of the file's past. **Fail-closed**: if the re-encode fails or outgrows the file limit on a file that carried metadata, the file is not attached at all — the honest failure is a notice, never a silent send of location data. Scope, decided: SVG (no EXIF; never rasterised — its scripts are neutralised by the img-context viewer) and GIF (comment-only metadata surface; a re-encode would kill animation) pass through. Cost note: the re-encode is a one-time decode+encode of the image on the sender's machine at attach time — imperceptible for photos, and the attachment slip shows the re-encoded size, so what you see is what you seal.
  - **In-app viewing** (round 29; `src/components/cc/file-viewer.tsx`): every file opens **in the app** — images zoom, video and audio play, PDFs render, text/CSV/binary are read right there — nothing is handed to the browser's own viewer and nothing is force-downloaded. The decrypted bytes decode once to memory and render through **revocable blob: URLs** (minted in an effect keyed on the message id, revoked on switch/unmount): nothing is fetched, nothing is written. **A view-once file has no download anywhere** — the Download button exists only for regular files, PiP and the context menu are suppressed on view-once media, and the viewer states the policy outright ("There is no download for this file, from anyone, by design"). PDFs render through **pdfjs-dist onto a `<canvas>`** (the worker vendored at `public/pdf.worker.min.mjs`, served from `/pdf.worker.min.mjs`) precisely because a canvas carries no browser PDF toolbar — and therefore no save button; if pdf.js cannot load the document, regular files fall back to an `<iframe>` honestly, but **view-once PDFs stay sealed** rather than expose the browser toolbar's save button (the notice says exactly that). **HTML is never executed** — rendered as escaped source ("Shown as source — HTML is never executed here"); **SVG is only ever shown through an `<img>`**, the context where its scripts cannot run; text is capped at 100 KB on screen while copy takes the whole file; CSV parses quote-aware into a capped table; unknown binaries get a metadata card and a hex dump of their first 512 bytes.
...
## 7. The porch doctrine — marketing surfaces

*Written the hard way (Task 40, the operator's "Task 39: The Porch"): the app passed every design review; the first porch regressed to the 2025 template mean — a three-zone layout, a spec-sheet flex, ALL-CAPS CTAs, an aphorism wall, an embedded demo chat. The taste already existed in the product; the porch simply hadn't borrowed it. Output regresses to the mean of its training data unless a constraint actively holds it off the mean — these are the constraints. For a privacy product the AI-look is a trust signal pointed the wrong way: strangers judge security tools by whether they feel deliberate, and a template says "generated."*

- **The swap test** — the acceptance test that would have caught it: cover the wordmark; if the page could sell any other privacy product, redo it. Identifiability comes from the product's own furniture — its mark, its verbs, its staging, its copy — never from an arrangement of zones.
- **The porch rule:** behavior claims on the porch; algorithms in the cabinet. *"The relay cannot read a single frame"* is a behavior claim in plain words — checkable. A cipher's name is vocabulary, and vocabulary is a flex. Algorithm names live in README / SECURITY / the protocol reader, linked from the footer.
- **One aphorism per product.** The voice already chose: **A conversation that leaves no trace.** No other tagline ships on any surface — parallel-batch aphorism walls are the loudest generated tell.
...
- **The porch wears the product's clothes:** the same verb system — sentence case, forest primary, quiet secondary; never the ALL-CAPS tracking-wide button — the same paper, the same mark, the same fonts the product itself wears. The porch is the product's front room, not a brochure about it.
- **Documents may look like documents.** INDEX rails, ledgers, edition lines and section kickers are honest furniture *in a reader*. The crime was the landing wearing the document's clothes. The landing is a letter on a desk — and the desk around it stays empty, because the emptiness is the material.

*Amended in round 3 (Task 41), the hard way again: the staging itself became the tell.* A prop sheet rotated on a desk, a struck-through word in the headline, doubled grain, a performed sealing gesture — authenticity theater, which is precisely what generated pages perform when asked to feel human. Apple would not stage a letter on a desk; nobody at Google would rotate a card half a degree. The bar, in house terms: **the porch performs nothing.** The page *is* the paper — flat, centered, set in the house's own type at porch scale, wearing the product's actual furniture: its header, its entrance, its breathing mark, its verb system, its footer caption. Identity survives through what the product already owns — the paper, the forest, the ink drop, the one aphorism — never through an arrangement of props. And the porch adds zero CSS of its own: every class it wears is a house class. If a future porch needs a new class to be itself, that is the smell of staging.

*Amended in round 4 (Task 42): the reader had learned the wrong lesson.* "Documents may look like documents" was taken as license for a costume — its own display fonts, "field notes" conceits, §-numbered mono kickers, ALL-CAPS labels, bench tests, receipts, colophons, pull quotes. A different website, wearing a different font system, that could document any privacy product on earth. The rule as it stands: **a document page wears the house's own clothes** — the same Lora, Inter and Plex Mono the product uses, the same paper, the same hairlines, the app's own header. Plain headings in sentence case. Labels in sentence case Inter, never mono ALL-CAPS. And the copy speaks plainly: no conceits, no "field notes," no "receipts," no clever-fragment titles — a heading says what the section is, a sentence says the thing. The reader is allowed one table, numbered steps, and code in mono, because those are what documents actually use.*

*Amended in round 5 (Task 43): the prose itself was the last costume.* Flat sentences had been hiding behind rhythm: em-dashes carrying tone instead of meaning, "minted" for generated, "sealed" for encrypted, "stretched" for derived, "standing gates" for runs on every push, "residuals" for known limits. The rule as it stands: **public copy carries zero em-dashes** (the one exception is the tagline's own dash), and vocabulary says the plain thing. Standard crypto terms survive ("key stretching," AES-256-GCM, argon2id); metaphors do not. A flattened sentence also leaves its numbers nowhere to hide: every number in public copy must survive on its own against the code. Round 5's catch: "room lifetimes up to a day" was wrong by a factor of thirty (the picker says minutes to 30 days, or until burned), and message timers go down to 5 seconds, not 15 (custom floor, `TTL_MIN_SEC`). The covenant sentence ("claims are checked against the code, and known limits are written down rather than hidden") ships in exactly one place, the author section, and even there it ships flat.

**Surfaces (one route, three faces):** `/` — the front room: the page is the paper. Flat, centered, nothing rotated, nothing floating, no shadow, no gesture — the breathing mark, the Lora headline at porch scale, one Inter meta line, two house verbs, the trust caption, the cabinet in the footer. `/?read=1` — the protocol page: a plain document in house clothes — Lora headings, Inter prose, Plex Mono for actual technical strings, an "On this page" line, hairline sections, one table, one live example. `/?app=1` — the product itself, unchanged.
